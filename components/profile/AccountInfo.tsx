      ...
      if (userId && userId !== user?.id) {
        try {
          ...
          if (userData.created_at) {
            ...
            setMemberSince(format(createdDate, isArabic ? 'MMMM yyyy' : 'MMMM yyyy', { locale: isArabic ? ar : undefined }));
          }
          
          if (userData.last_active) {
            const lastActiveDate = new Date(userData.last_active);
            setLastSignIn(formatDistanceToNow(lastActiveDate, { 
              addSuffix: true,
              locale: isArabic ? ar : undefined
            }));
          } else {
            setLastSignIn(isArabic ? 'أبداً' : 'Never');
          }
        ...
        if (user.last_sign_in_at) {
          ...
          setLastSignIn(formatDistanceToNow(lastSignInDate, { 
            addSuffix: true,
            locale: isArabic ? ar : undefined
          }));
        } else {
